...
class MangaCallFactory(
    ...
    override fun newCall(request: okhttp3.Request): okhttp3.Call {
        ...
        val rule = ruleId?.let { runBlocking { ruleManager.getRule(it) } }
        ...
        rule?.let {
            ...
            if (!it.globalConfig.headerScript.isNullOrBlank()) {
                val dynamicHeaders = JsExtensionClass.jsToJavaObject(SCRIPT_ENGINE.eval(it.globalConfig.headerScript!!,bindings))
        ...
        return client.newCall(request.newBuilder().headers(newRequestBuilder.removeAll("X-Internal-Rule-Id").build()).build())
    }
}