...
class BookshelfViewModel(
    ...
    override fun handleIntent(intent: BookshelfUiIntent) {
        when (intent) {
            ...
            is BookshelfUiIntent.DeleteSelectedBooks -> {} //handleDeleteSelectedBooks()
        }
    }
    ...
    private fun handleExportSelectedBooks() {
        val state = uiState.value
        val selectedUrls = state.selectedBooks // 拿到当前所有勾选的书籍 URL

        if (selectedUrls.isEmpty()) return

        // 开启刷新指示器（让界面转个小圈圈，提示正在打包中）
        updateState { copy(isRefreshing = true) }

        viewModelScope.launch {
            try {
                // 1. 动态获取当前电脑系统的“桌面（Desktop）”绝对路径
                // Windows: C:\Users\用户名\Desktop
                // macOS: /Users/用户名/Desktop
                ...
                selectedUrls.forEach { url ->
                    val book = state.books.find { it.url == url }
                    if (book != null) {
                        // 💡 清洗文件名：防止书名里包含 \ / : * ? " < > | 等 Windows 禁忌字符导致崩溃
                        val safeTitle = book.title.replace(Regex("[\\\\/:*?\"<>|]"), "_")
                        val destinationZipPath = "$desktopPath/$safeTitle.zip"

                        // 💡 调用 KMP 抽象接口进行打包！底层在桌面端会自动走 JVM 压缩流
                        val success = bookPackager.packageBookToZip(book.url, destinationZipPath)
                        if (success) {
                            successCount++
                        }
                    }
                ...
                // 3. 打包结束：关闭 Loading，自动退出编辑状态，清空选择
                updateState {
                    copy(isRefreshing = false, isSelectionMode = false, selectedBooks = emptySet())
                }

                if (successCount > 0) {
                    sendEffect(BookshelfUiEffect.ShowToast("成功将 $successCount 本书打包导出至电脑桌面！"))
                } else {
                    sendEffect(BookshelfUiEffect.ShowToast("导出失败，请检查是否已缓存章节"))
                }
...
sealed class BookshelfUiIntent : UiIntent {
    ...
    data object ExportSelectedBooks : BookshelfUiIntent()
